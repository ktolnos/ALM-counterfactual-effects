...
class ModelContrafactualPrior(nn.Module):
    ...
    def _calculate_diff_mean_std(self, z, action):
        ...
        ec_mean, ec_std = _get_mean_std(self.effect_contrafactual_model(x), self.std_min, self.std_max)
        ic_mean, ic_std = _get_mean_std(self.independent_change_model(z), self.std_min, self.std_max)
        return ec_mean+ic_mean, ec_std + ic_std

    def ic(self, z):
        return _independent_normal_mean_std(*_get_mean_std(self.independent_change_model(z), self.std_min, self.std_max))

    def ec(self, z, a):
        return _independent_normal_mean_std(*_get_mean_std(self.effect_contrafactual_model(z, a, self.std_min, self.std_max)))
    ...
    def forward(self, actual_z, actual_action, next_actual_z, contr_z, contr_action):
        ...
        if actual_z is contr_z:
            ...
            actual_effect, std_a = _get_mean_std(self.effect_contrafactual_model(z_a_act),self.std_min, self.std_max)
            z_a_contr = torch.cat((contr_z, contr_action), dim=-1)
            contr_effect, std_c = _get_mean_std(self.effect_contrafactual_model(z_a_contr),self.std_min, self.std_max)
        else:
